...
- `ChatController` — constructed as `new ChatController(deps: ControllerDeps)`; `deps` is stored as a `private readonly` parameter property. The constructor is pure state initialization: it resolves `cwd`/`workspaceName` per the fallbacks above and seeds `#session` with `agentStatus: "starting"`, `isStreaming: false`, `isCompacting: false`, `fastModeEnabled: false`, `fastModeActive: false`, `autoCompactionEnabled: true`, `steeringMode: "one-at-a-time"`, `followUpMode: "one-at-a-time"`, `tokensPerSecond: null`, `queuedMessageCount: 0`. **No process is spawned in the constructor** — that happens in `start()`.
  ...
  - `openFile(target: string, line?: number, column?: number): Promise<void>` — a trailing separator is trimmed first (listings mark directories as `tools/`, but no editor URI carries that; a bare root like `/` or `c:\` keeps its own). A path that stats as a directory is handed to the `revealInExplorer` command and nothing else happens — `line`/`column` cannot apply — because `openTextDocument` rejects a directory outright and the dispatch catch would turn that into an error toast. Transcript path links are not all files: a `read` of a directory, a glob row, and a grep `# dir` header all post one. Otherwise the file opens as a preview editor, and when `line` is given, converts 1-based line/column to a 0-based `Position` (clamped at `0`, `column` defaults to `1`), sets the selection, and reveals with `InCenterIfOutsideViewport`. A stat failure counts as "not a directory", so a missing path still produces VS Code's own diagnosis rather than an invented one.
...
- `workspace` — `workspaceFolders: undefined`; `getConfiguration(_section)` → `{ get<T>(key, fallback?): T | undefined }` reading from the module-level `CONFIG` (the section argument is **ignored**; keys are looked up unprefixed, so `omp.model` is read as `model`); `asRelativePath(value)` → `String(value)`; `registerTextDocumentContentProvider()` and `onDidChangeConfiguration()` return no-op `Disposable`s. `fs.stat(uri)` is **real** — it calls `node:fs/promises` `stat` on `uri.fsPath` and returns `{ type: FileType.Directory | FileType.File }`, rejecting for a missing path, because the only useful answer to "is this a directory" comes from the actual filesystem. **Throws:** `openTextDocument()` — `"openTextDocument is not available in the harness"`; `fs.readFile()` — `"fs.readFile is not available in the harness"`.
...
- `commands` — `registerCommand()` returns a no-op `Disposable`; `executeCommand(command, ...args)` records `{ command, args }` into the module-level `executed` array and resolves `undefined`, so a test can assert which command a handler invoked (`smoke-open-file.ts` uses it for `revealInExplorer`). The array is exposed as `harness.executed` and is cleared by the test, not the stub.
- Editor value types — `Position`, `Range`, `Selection` (an empty subclass of `Range`), `TextEditorRevealType` with only `InCenterIfOutsideViewport: 2`, and `FileType` (`Unknown: 0`, `File: 1`, `Directory: 2`, `SymbolicLink: 64`) matching the real bit values, since callers test it with `&`.
